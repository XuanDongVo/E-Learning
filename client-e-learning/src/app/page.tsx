      ...
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-12">
        ...
        <section className="relative py-20 sm:py-28 lg:py-32">
          ...
          <div className="relative max-w-3xl">
            <p className="mb-5 text-sm font-extrabold uppercase tracking-[0.2em] text-primary">Learn with purpose</p>
            ...
            <p className="mt-7 max-w-2xl text-base leading-8 text-neutral-muted sm:text-lg">LearnVerse brings lessons, practice, assignments, and progress into one shared rhythm for teachers and students.</p>
          </div>
        </section>
          ...
          {audiences.map((audience) => {
            ...
            return (
              <article key={audience.href} className="group relative overflow-hidden rounded-[var(--radius-lg)] border border-border-color bg-card-bg p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:p-9">
                ...
                <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-neutral-subtle">{audience.eyebrow}</p>
                <h2 className="mt-3 text-3xl font-extrabold tracking-tight">{audience.title}</h2>
                <p className="mt-4 max-w-md text-sm leading-7 text-neutral-muted">{audience.description}</p>
                <Link href={audience.href} className="mt-8 inline-flex items-center gap-2 text-sm font-extrabold text-primary transition group-hover:gap-3">{audience.button}<ArrowRight className="h-4 w-4" /></Link>
              </article>
            );
          })}
        </section>
        <footer className="border-t border-border-color py-6 text-xs text-neutral-subtle">A focused learning space for classes, practice, and progress.</footer>
      </div>
    </main>
  );
}